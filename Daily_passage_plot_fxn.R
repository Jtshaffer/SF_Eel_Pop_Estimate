...
daily_passage_plot<- function(Data1= XYZ, Data2= ZYX, DateBreaks = "1 day", dual_axis = T,startdate="2023-10-10", enddate=today()){
  if (dual_axis==T) {
  ...
  plot <- ggplot() +
    ...
    geom_point(aes(x=Data2$Date, y = Data2$Corrected.Daily.Net ),size = 1.75) +
  ...
  plot
    ...
    max <- as.POSIXct(strptime(startdate,format = "%Y-%m-%d"))
    min <- as.POSIXct(strptime(enddate,format = "%Y-%m-%d"))
    
    plot1 <- ggplot() +
      geom_line(data = Data1, aes(x =dateTime, y = Flow_Inst), color ='blue')+
